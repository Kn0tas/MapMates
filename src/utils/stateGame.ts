...
import { pickRandom, shuffle } from "./game";
...
export const buildStateRound = (
  pool: StateGeometry[],
  excludeCode?: string,
  excludeCodes: string[] = [],
): RoundPayload => {
  const workingPool = pool.length ? pool : allStates;
  const excludeSet = new Set(excludeCodes);
  if (excludeCode) {
    excludeSet.add(excludeCode);
  }

  let candidatePool = workingPool.filter((state) => !excludeSet.has(state.code));

  if (!candidatePool.length) {
    const withoutLast = excludeCode
      ? workingPool.filter((state) => state.code !== excludeCode)
      : workingPool;
